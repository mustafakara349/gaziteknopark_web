...
import AdminAnnouncementCategoryModal from "../../components/admin/announcements/AdminAnnouncementCategoryModal";
import AdminAnnouncementFormModal from "../../components/admin/announcements/AdminAnnouncementFormModal";
...
export default function AdminAnnouncementsPage() {
  ...
  useEffect(() => {
    fetchAnnouncements();
  }, [page, search, categoryId, isActive, date, isPinned]);

  const fetchCategories = async () => {
    try {
      const response = await adminAxios.get("/announcement-categories");
  ...
  const fetchAnnouncements = async () => {
    ...
    try {
      ...
      const response = await adminAxios.get("/announcements", { params });
      
      let fetchedData = response.data;
      if (isActive !== "") {
        fetchedData = fetchedData.filter(a => a.isActive === (isActive === "true"));
      }
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-[#0F172A] tracking-tight">Duyuru Yönetimi</h1>
          <p className="text-gray-500 mt-2 text-sm max-w-xl">
            Duyuru ekleme, düzenleme, silme ve kategori yönetimi işlemlerini buradan gerçekleştirebilirsiniz.
          </p>
        </div>
      ...
      <div className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-6 flex flex-wrap gap-4 items-center bg-gray-50/50 border-b border-gray-100">
          ...
          <div className="relative">
            <select 
              value={isPinned}
              onChange={(e) => { setIsPinned(e.target.value); setPage(1); }}
              className="appearance-none pl-4 pr-10 py-2.5 bg-white border border-gray-200 rounded-full text-sm font-medium text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 cursor-pointer"
            >
              <option value="">Sabitleme Durumu</option>
              <option value="true">Sabitlenmiş</option>
              <option value="false">Sabitlenmemiş</option>
            </select>
          </div>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900 truncate max-w-[150px] sm:max-w-[200px] xl:max-w-[300px]">
                      <div className="flex items-center gap-2">
                        {item.isPinned && <Pin className="w-3 h-3 text-blue-500 shrink-0" fill="currentColor" />}
                        {item.title}
                      </div>
                      <div className="text-xs text-gray-400 font-normal mt-0.5 truncate">{item.summary || "-"}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => window.open(`/duyurular/${item.slug}`, '_blank')}
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-white">
          <div className="text-sm text-gray-500">
            Toplam <span className="font-medium text-gray-900">{totalCount}</span> duyurudan <span className="font-medium text-gray-900">{announcements.length > 0 ? (page - 1) * 10 + 1 : 0}-{Math.min(page * 10, totalCount)}</span> arası gösteriliyor
          </div>
      ...
      {/* Modals */}
      <AdminAnnouncementCategoryModal 
